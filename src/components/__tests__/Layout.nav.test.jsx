...
// Asserts exactly one NAV entry with id 'institute-outreach' (non-empty label +
// icon) and a matching renderSection() case, without overriding the app's
// noindex/password-gate behaviour.
//
// The entry lives in the 'growth' ("Get Clients") group: Institute Outreach and
// Lawns & Banquets are both client-acquisition surfaces, so they sit together
// there rather than in the general 'execution' group where this originally lived.
...
describe('Layout — Institute Outreach nav registration', () => {
  it('registers exactly one NAV entry with id institute-outreach in the Get Clients group', () => {
    ...
    // growth ("Get Clients") group, all on the same NAV line.
    ...
    expect(navLine).toMatch(/group:\s*'growth'/);
  });

  it('groups Lawns & Banquets alongside it under the same Get Clients group', () => {
    const navLine = layoutSource
      .split('\n')
      .find((l) => l.includes("id: 'lawns-outreach'") && l.includes('group:'));
    expect(navLine).toBeDefined();
    expect(navLine).toMatch(/group:\s*'growth'/);
    expect(layoutSource).toMatch(
      /case\s*'lawns-outreach':\s*return\s*<LawnsOutreach\s*\{\.\.\.props\}\s*\/>;/
    );
  });
